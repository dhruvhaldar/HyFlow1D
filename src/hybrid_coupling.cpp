...
HybridDomain::HybridDomain(std::unique_ptr<Solver1D> left, std::unique_ptr<Solver1D> right)
    ...
    // Security: Validate pointers to prevent Null Pointer Dereference (CWE-476)
    if (!left_domain) {
        throw std::invalid_argument("HybridDomain: Left solver pointer cannot be null.");
    }
    if (!right_domain) {
        throw std::invalid_argument("HybridDomain: Right solver pointer cannot be null.");
    }
}
